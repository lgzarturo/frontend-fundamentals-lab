...
export function mitItemTemplate(task) {
  ...
  const tagsHtml = (task.tags || [])
    .map(tag => html`<span class="px-1.5 py-0.5 text-xs rounded bg-xp-primary/20 text-xp-primary">${tag}</span>`)
    .join('')

  return html`
    <div class="flex items-start gap-3 p-3 bg-gray-50 dark:bg-xp-darker rounded-lg">
      <input type="checkbox" ${task.done ? 'checked' : ''} data-action="toggle-task" data-task-id="${task.id}" class="mt-0.5 accent-xp-primary" />
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-1.5 flex-wrap">
          <span class="text-sm">${icon}</span>
          <span class="font-medium text-sm ${task.done ? 'line-through text-gray-400' : ''}">${task.title}</span>
        </div>
  ...
  return html`
    <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-xp-darker rounded-lg">
      <div class="flex items-center gap-2">
        ...
        <span class="text-sm font-medium ${completed ? 'line-through text-gray-400' : ''}">${habit.title}</span>
      </div>
...
export function activityItemTemplate(item, i18n) {
  ...
  return html`
    <div class="flex items-center gap-3 py-2 border-b border-gray-100 dark:border-xp-primary/10 last:border-0">
