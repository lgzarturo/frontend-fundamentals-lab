...
export const HABIT_TEMPLATES = [
  { title: '🌅 Despertar sin snooze', description: 'Despertar a la hora objetivo sin tocar snooze', color: '#fbbf24' },
  { title: '💧 Hidratarse (500ml agua)', description: 'Beber 500ml de agua con limón al despertar', color: '#60a5fa' },
  { title: '🧘 Meditación estoica (10 min)', description: 'Meditación matutina y diario', color: '#a78bfa' },
  { title: '🏃 Rutina de movilidad', description: '15-20 minutos de estiramientos y calistenia', color: '#34d399' },
  { title: '⭐ Definir 3 MITs', description: 'Planificar las 3 tareas más importantes en el desayuno', color: '#fbbf24' },
  { title: '🎯 Completar primer bloque de trabajo profundo', description: 'Sesión de trabajo enfocada de 60 minutos', color: '#00ff88' },
  { title: '📚 Bloque de aprendizaje (30-45 min)', description: 'Tiempo dedicado a aprender nuevas habilidades', color: '#f472b6' },
  { title: '📝 Revisión de fin de día', description: 'Revisar logros y planear mañana', color: '#94a3b8' },
  { title: '🌙 Atardecer digital (6 PM)', description: 'Desconectarse de pantallas a las 6 PM', color: '#818cf8' },
  { title: '😴 Preparación para dormir (9 PM)', description: 'Iniciar rutina de sueño, dormir a las 11 PM', color: '#6366f1' }
]
...
export function habitCardTemplate(habit, i18n) {
  ...
  const toggleLabel = isToday
    ? (i18n?.getMessage('habits.markIncomplete') || 'Desmarcar')
    : (i18n?.getMessage('habits.markComplete') || 'Completar')

  const weekDots = weekRecords.map((record, idx) => html`
    <div class="flex flex-col items-center gap-1">
      <span class="text-xs text-gray-400">${DAY_LABELS[idx % 7]}</span>
      <div class="w-6 h-6 rounded-full border-2 flex items-center justify-center ${record.completed
        ? 'border-transparent'
  ...
  return html`
    <div class="bg-white dark:bg-xp-card rounded-xl p-5 border-2 border-gray-200 dark:border-xp-primary/20">
      <div class="flex items-start justify-between mb-3">
        ...
        <button
          ...
          aria-label="${i18n?.getMessage('app.common.delete') || 'Eliminar'}"
        >✕</button>
      </div>

      <div class="flex items-center gap-2 mb-4">
        <span class="text-xp-warning">🔥</span>
        <span class="text-sm font-semibold">${habit.streak} ${i18n?.getMessage('habits.streak') || 'días seguidos'}</span>
        <div class="w-2 h-2 rounded-full ml-auto flex-shrink-0" style="background-color: ${habit.color}"></div>
      </div>
      ...
      <button
        ...
        data-habit-id="${habit.id}"
        class="w-full py-2.5 rounded-lg font-semibold text-sm transition-colors ${isToday
          ? 'text-xp-darker'
...
export function emptyHabitsTemplate(i18n) {
  return html`
    <div class="col-span-full text-center py-12">
      ...
      <button data-action="create-habit" class="px-6 py-3 bg-xp-primary text-xp-darker font-bold rounded-lg hover:bg-xp-primary/80 transition-colors">
...
export function habitTemplatesModalTemplate(i18n) {
  const templateCards = HABIT_TEMPLATES.map(t => html`
    <button
    ...
    >
      ...
      <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 ml-5">${t.description}</p>
    </button>
  `).join('')
  ...
  return html`
    <div class="p-6">
      <h3 class="text-2xl font-bold mb-4">
        ${i18n?.getMessage('habits.modal.title') || 'Agregar Hábito'}
      </h3>

      <div class="mb-6">
        <h4 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">
          ${i18n?.getMessage('habits.modal.templates') || 'Plantillas'}
        </h4>
      ...
      <div>
        <h4 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">
          ${i18n?.getMessage('habits.modal.custom') || 'Hábito personalizado'}
        </h4>
        <form id="create-habit-form" class="space-y-3">
          <div>
            <label class="block text-sm font-semibold mb-1">
              ${i18n?.getMessage('habits.modal.titleLabel') || 'Título *'}
            </label>
            <input
              type="text"
              name="title"
              ...
              placeholder="${i18n?.getMessage('habits.modal.titlePlaceholder') || 'ej., Leer 20 minutos'}"
            />
          </div>
          <div>
            <label class="block text-sm font-semibold mb-1">
              ${i18n?.getMessage('habits.modal.descriptionLabel') || 'Descripción'}
            </label>
            <input
              type="text"
              name="description"
              class="w-full px-4 py-2 rounded-lg border-2 border-gray-200 dark:border-xp-primary/20 bg-white dark:bg-xp-darker focus:outline-none focus:border-xp-primary"
              placeholder="${i18n?.getMessage('habits.modal.descriptionPlaceholder') || 'Opcional'}"
            />
          </div>
          <div>
            <label class="block text-sm font-semibold mb-1">
              ${i18n?.getMessage('habits.modal.colorLabel') || 'Color'}
            </label>
            <input
              type="color"
              name="color"
